...
public class DecipherBruteForce {
    ...
    private static HashMap<Integer, Integer> labels = new HashMap<>();
    ...
    protected static String deBruteForce(String str1, String str2) {
        ...
        for (int s = 0; s < Solution.ALPHABET.length; s++) {   //прохожу дешифратором по тексту 40 раз.
            ...
            int count = 0;
            for (int i = 0; i < symbol.length; i++) {
                if (symbol[i] == ' ') {                   //ищу сколько всего пробелов при каждом проходе
                    count++;
                }
                labels.put(key, count);                  //записываю в мапу ключ и количество пробелов
            }
            key++;
        }
        Integer key1 = 0;
        Integer value1 = 0;
        for (Map.Entry<Integer, Integer> pair : labels.entrySet()) {  //ищу в мапе самое большое количество пробелов
        ...
        char[] symbol2 = new char[encryptText.length()];            //по новому ключу расшифровка, беру из мапы ключ
        for (int i = 0; i < symbol2.length; i++) {          //у которого самое большое значение элемента
